# Tài liệu dự án — Solace (Nơi nỗi lòng được cất giữ)
...
Solace định vị mình vào đúng khoảng trống đó: **ẩn danh nhưng an toàn, nhẹ nhàng nhưng có trách nhiệm**.
...
Ngôn ngữ thị giác của Solace xoay quanh 3 ý niệm: **vũ trụ ban đêm**, **đại dương sâu**, và **ánh sáng ấm**. Toàn bộ giao diện dùng nền tối (dark mode mặc định và duy nhất hiện tại) để tạo cảm giác riêng tư, an toàn, giảm cảm giác "phơi bày".
...
|---|---|---|
| `AuthContext` | Phiên đăng nhập mô phỏng (email, masked email, pending magic link) | `solace:auth` |
| `AppStateContext` | Identity, vibe, mood/mood history, draft đang viết, toàn bộ story, reaction, giới hạn reaction/ngày, mốc thời gian mở app lần đầu | `solace:v2` + `solace:all-signals` |
| `LanguageContext` | Ngôn ngữ hiện tại + hàm `t()` tra cứu từ điển | `solace:lang` |
| `NotificationContext` | Danh sách thông báo + cài đặt bật/tắt theo loại | `solace:notifications` |

Ngoài ra: `solace:feedback-nudge-dismissed`, `solace:feedback-submissions`.
...
4. **Namespace localStorage đã tính trước migration** (`STORAGE_KEY = "solace:v2"`), nhưng chưa có cơ chế đồng bộ lên server có consent người dùng.
...
Từ một "nơi giải toả cảm xúc ẩn danh", Solace có tiềm năng phát triển thành **một hệ sinh thái chăm sóc sức khoẻ tinh thần nhẹ nhàng cho Gen Z Việt Nam**, gồm 3 lớp:
